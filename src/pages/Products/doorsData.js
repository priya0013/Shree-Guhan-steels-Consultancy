export const doorProducts = [
  {
    id: 1,
    image: "/assets/images/doors/d1.jpg",
    name: "Signature Steel Security Door",
    model: "DG-701",
    description: "Premium entry door with multi-point locking and thermal insulation",
    price: "₹28,500",
    category: "Security Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1200 x 2400 x 60 mm"],
    sizes: ["880", "980", "1180"],
    colors: ["#2b2b2b", "#1a1a1a", "#3d3d3d", "#5a5a5a"],
    material: "Heavy-duty galvanized steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "Multi-point (6-8 points)",
      insulation: "0.40 U-value",
      soundReduction: "30-35 dB",
      fireRating: "Optional 60/90 min"
    },
    gallery: [
      "/assets/images/doors/d1.jpg",
      "/assets/images/doors/d2.jpg",
      "/assets/images/doors/d3.jpg",
      "/assets/images/doors/d4.jpg",
      "/assets/images/doors/d5.jpg"
    ],
    features: [
      "Multi-point locking system",
      "Thermal insulation layer",
      "Weather-resistant seals",
      "Premium finish options",
      "5-year warranty"
    ]
  },
  {
    id: 2,
    image: "/assets/images/doors/d2.jpg",
    name: "Modern Gloss Black Door",
    model: "DG-702",
    description: "Sleek modern door with premium gloss black finish",
    price: "₹26,800",
    category: "Modern Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#1a1a1a", "#2b2b2b", "#3d3d3d"],
    material: "Powder-coated steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.35 U-value",
      soundReduction: "28-32 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d2.jpg",
      "/assets/images/doors/d3.jpg",
      "/assets/images/doors/d4.jpg",
      "/assets/images/doors/d5.jpg",
      "/assets/images/doors/d6.jpg"
    ],
    features: [
      "Decorative panel design",
      "Anti-rust galvanized coating",
      "Smooth operation with quality hinges",
      "Low maintenance finish",
      "Available in 8 colors"
    ]
  },
  {
    id: 3,
    image: "/assets/images/doors/d3.jpg",
    name: "Luxury Matte Black Door",
    model: "DG-703",
    description: "Premium matte black finish with contemporary styling",
    price: "₹32,000",
    category: "Premium Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#1a1a1a", "#2b2b2b", "#3d3d3d"],
    material: "Powder-coated steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "7-point system",
      insulation: "0.42 U-value",
      soundReduction: "32-38 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d3.jpg",
      "/assets/images/doors/d4.jpg",
      "/assets/images/doors/d5.jpg",
      "/assets/images/doors/d6.jpg",
      "/assets/images/doors/d7.jpg"
    ],
    features: [
      "Matte black finish (no fingerprints)",
      "Scratch-resistant coating",
      "Premium soft-close mechanism",
      "Thermal break technology",
      "5-year warranty"
    ]
  },
  {
    id: 4,
    image: "/assets/images/doors/d4.jpg",
    name: "Elegant Glass Panel Door",
    model: "DG-704",
    description: "Modern door with safety glass panels for light and elegance",
    price: "₹31,500",
    category: "Glass Panel Doors",
    dimensions: ["850 x 2100 x 50 mm", "900 x 2200 x 55 mm", "1000 x 2400 x 60 mm"],
    sizes: ["830", "880", "980"],
    colors: ["#3d3d3d", "#2c2c2c", "#4a4a4a"],
    material: "Steel with safety tempered glass",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "6-point system",
      insulation: "0.38 U-value",
      soundReduction: "30-35 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d4.jpg",
      "/assets/images/doors/d5.jpg",
      "/assets/images/doors/d6.jpg",
      "/assets/images/doors/d7.jpg",
      "/assets/images/doors/d8.jpg"
    ],
    features: [
      "Tempered safety glass panels",
      "Enhanced light transmission",
      "Privacy frosted glass options",
      "Steel frame support",
      "Easy maintenance"
    ]
  },
  {
    id: 5,
    image: "/assets/images/doors/d5.jpg",
    name: "Industrial Steel Portal",
    model: "DG-705",
    description: "Heavy-duty industrial door for commercial and warehouse use",
    price: "₹38,500",
    category: "Industrial Doors",
    dimensions: ["1000 x 2200 x 70 mm", "1200 x 2400 x 80 mm", "1500 x 2500 x 90 mm"],
    sizes: ["980", "1180", "1480"],
    colors: ["#2b2b2b", "#4a4a4a", "#636363"],
    material: "Industrial-grade steel",
    specifications: {
      frameThickness: "2.1-2.5 mm",
      lockPoints: "8-point system",
      insulation: "0.30 U-value",
      soundReduction: "35-40 dB",
      fireRating: "120 min optional"
    },
    gallery: [
      "/assets/images/doors/d5.jpg",
      "/assets/images/doors/d6.jpg",
      "/assets/images/doors/d7.jpg",
      "/assets/images/doors/d8.jpg",
      "/assets/images/doors/d9.jpg"
    ],
    features: [
      "Heavy-duty construction for high traffic",
      "Reinforced hinges and locks",
      "Self-closing mechanism",
      "Fire-resistant option available",
      "Customizable size for large openings"
    ]
  },
  {
    id: 6,
    image: "/assets/images/doors/d6.jpg",
    name: "Classic Charcoal Door",
    model: "DG-706",
    description: "Timeless charcoal finish with traditional styling",
    price: "₹23,500",
    category: "Classic Doors",
    dimensions: ["900 x 2100 x 50 mm", "950 x 2150 x 52 mm", "1000 x 2200 x 55 mm"],
    sizes: ["880", "930", "980"],
    colors: ["#505050", "#3d3d3d", "#4a4a4a", "#5a5a5a"],
    material: "Galvanized steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.39 U-value",
      soundReduction: "28-33 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d6.jpg",
      "/assets/images/doors/d7.jpg",
      "/assets/images/doors/d8.jpg",
      "/assets/images/doors/d9.jpg",
      "/assets/images/doors/d10.jpg"
    ],
    features: [
      "Traditional panel design",
      "Charcoal color finish",
      "Durable powder coating",
      "Compatible with standard frames",
      "Budget-friendly option"
    ]
  },
  {
    id: 7,
    image: "/assets/images/doors/d7.jpg",
    name: "Silver Metallic Door",
    model: "DG-707",
    description: "Modern silver metallic finish with sleek design",
    price: "₹26,800",
    category: "Modern Doors",
    dimensions: ["850 x 2100 x 48 mm", "900 x 2200 x 50 mm", "1000 x 2400 x 55 mm"],
    sizes: ["830", "880", "980"],
    colors: ["#a0a0a0", "#808080", "#909090"],
    material: "Anodized aluminum-effect steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.40 U-value",
      soundReduction: "29-33 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d7.jpg",
      "/assets/images/doors/d8.jpg",
      "/assets/images/doors/d9.jpg",
      "/assets/images/doors/d10.jpg",
      "/assets/images/doors/d11.jpg"
    ],
    features: [
      "Contemporary silver metallic finish",
      "Light-reflective surface",
      "Modern minimalist design",
      "Easy cleaning and maintenance",
      "Corrosion-resistant coating"
    ]
  },
  {
    id: 8,
    image: "/assets/images/doors/d8.jpg",
    name: "Midnight Blue Steel Door",
    model: "DG-708",
    description: "Unique blue-tinted steel door for distinctive interiors",
    price: "₹27,500",
    category: "Colored Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#1a3a4a", "#2a4a5a", "#3a5a6a"],
    material: "Color-coated steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.35 U-value",
      soundReduction: "28-32 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d8.jpg",
      "/assets/images/doors/d9.jpg",
      "/assets/images/doors/d10.jpg",
      "/assets/images/doors/d11.jpg",
      "/assets/images/doors/d12.jpg"
    ],
    features: [
      "Custom blue color coating",
      "UV-resistant finish",
      "Distinctive appearance",
      "Premium finishing",
      "10-year color guarantee"
    ]
  },
  {
    id: 9,
    image: "/assets/images/doors/d9.jpg",
    name: "Executive Bronze Door",
    model: "DG-709",
    description: "Executive bronze finish for upscale residential and commercial spaces",
    price: "₹29,800",
    category: "Premium Doors",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#8B4513", "#704214", "#6B3410"],
    material: "Bronze-coated steel",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "6-point system",
      insulation: "0.38 U-value",
      soundReduction: "30-34 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d9.jpg",
      "/assets/images/doors/d10.jpg",
      "/assets/images/doors/d11.jpg",
      "/assets/images/doors/d12.jpg",
      "/assets/images/doors/d13.jpg"
    ],
    features: [
      "Executive bronze finish",
      "Anti-corrosion core",
      "Premium appearance",
      "Luxury styling",
      "Sophisticated color options"
    ]
  },
  {
    id: 10,
    image: "/assets/images/doors/d10.jpg",
    name: "Innovation Smart Door",
    model: "DG-710",
    description: "Smart-compatible door with integrated smart lock readiness",
    price: "₹36,500",
    category: "Smart Doors",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2b2b2b", "#1a1a1a", "#3d3d3d"],
    material: "Smart-integrated steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "7-point system",
      insulation: "0.40 U-value",
      soundReduction: "32-36 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d10.jpg",
      "/assets/images/doors/d11.jpg",
      "/assets/images/doors/d12.jpg",
      "/assets/images/doors/d13.jpg",
      "/assets/images/doors/d14.jpg"
    ],
    features: [
      "Pre-wired for smart locks",
      "IoT-ready design",
      "Mobile app compatibility",
      "Remote access capability",
      "Modern sleek design"
    ]
  },
  {
    id: 11,
    image: "/assets/images/doors/d11.jpg",
    name: "Thermal Insulated Series",
    model: "DG-711",
    description: "Maximum insulation for energy efficiency and noise reduction",
    price: "₹33,500",
    category: "Insulated Doors",
    dimensions: ["900 x 2100 x 70 mm", "1000 x 2200 x 75 mm", "1100 x 2400 x 80 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2c2c2c", "#3d3d3d", "#4a4a4a"],
    material: "Insulated steel composite",
    specifications: {
      frameThickness: "2.0-2.3 mm",
      lockPoints: "8-point system",
      insulation: "0.22 U-value",
      soundReduction: "40-45 dB",
      fireRating: "90 min optional"
    },
    gallery: [
      "/assets/images/doors/d11.jpg",
      "/assets/images/doors/d12.jpg",
      "/assets/images/doors/d13.jpg",
      "/assets/images/doors/d14.jpg",
      "/assets/images/doors/d15.jpg"
    ],
    features: [
      "Triple thermal insulation layer",
      "Sound dampening technology",
      "Weather sealing gaskets",
      "Energy efficiency rated",
      "Reduces heating/cooling costs"
    ]
  },
  {
    id: 12,
    image: "/assets/images/doors/d12.jpg",
    name: "Commercial Heavy Duty",
    model: "DG-712",
    description: "Built for high-traffic commercial environments",
    price: "₹35,000",
    category: "Commercial Doors",
    dimensions: ["1000 x 2200 x 65 mm", "1100 x 2400 x 70 mm", "1200 x 2500 x 75 mm"],
    sizes: ["980", "1080", "1180"],
    colors: ["#2b2b2b", "#4a4a4a"],
    material: "Industrial-grade steel",
    specifications: {
      frameThickness: "2.1-2.5 mm",
      lockPoints: "8-point system",
      insulation: "0.30 U-value",
      soundReduction: "35-40 dB",
      fireRating: "120 min optional"
    },
    gallery: [
      "/assets/images/doors/d12.jpg",
      "/assets/images/doors/d13.jpg",
      "/assets/images/doors/d14.jpg",
      "/assets/images/doors/d15.jpg",
      "/assets/images/doors/d16.jpg"
    ],
    features: [
      "Heavy-duty construction",
      "Commercial-grade locks",
      "Ice impact tested",
      "ADA compliant options",
      "Extended warranty"
    ]
  },
  {
    id: 13,
    image: "/assets/images/doors/d13.jpg",
    name: "Minimalist Flat Door",
    model: "DG-713",
    description: "Clean minimalist design with flat panel styling",
    price: "₹22,800",
    category: "Minimalist Doors",
    dimensions: ["850 x 2100 x 45 mm", "900 x 2200 x 50 mm", "1000 x 2400 x 55 mm"],
    sizes: ["830", "880", "980"],
    colors: ["#3d3d3d", "#2c2c2c", "#4a4a4a", "#5a5a5a"],
    material: "Flat-panel steel",
    specifications: {
      frameThickness: "1.5-1.7 mm",
      lockPoints: "4-point system",
      insulation: "0.42 U-value",
      soundReduction: "25-30 dB",
      fireRating: "15 min optional"
    },
    gallery: [
      "/assets/images/doors/d13.jpg",
      "/assets/images/doors/d14.jpg",
      "/assets/images/doors/d15.jpg",
      "/assets/images/doors/d16.jpg",
      "/assets/images/doors/d17.jpg"
    ],
    features: [
      "Flat panel minimalist design",
      "Clean modern aesthetic",
      "Budget-friendly",
      "Easy installation",
      "Contemporary styling"
    ]
  },
  {
    id: 14,
    image: "/assets/images/doors/d14.jpg",
    name: "Vintage Textured Door",
    model: "DG-714",
    description: "Textured surface design for vintage and classic interiors",
    price: "₹25,500",
    category: "Textured Doors",
    dimensions: ["900 x 2100 x 50 mm", "950 x 2150 x 52 mm", "1000 x 2200 x 55 mm"],
    sizes: ["880", "930", "980"],
    colors: ["#3d3d3d", "#505050", "#4a4a4a"],
    material: "Textured-surface steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.39 U-value",
      soundReduction: "28-33 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d14.jpg",
      "/assets/images/doors/d15.jpg",
      "/assets/images/doors/d16.jpg",
      "/assets/images/doors/d17.jpg",
      "/assets/images/doors/d18.jpg"
    ],
    features: [
      "Unique textured surface",
      "Vintage aesthetics",
      "Hides fingerprints well",
      "Premium feel",
      "Durable finish"
    ]
  },
  {
    id: 15,
    image: "/assets/images/doors/d15.jpg",
    name: "Diamond Plate Designer",
    model: "DG-715",
    description: "Diamond plate design for modern industrial interiors",
    price: "₹27,200",
    category: "Designer Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#4a4a4a", "#3d3d3d", "#5a5a5a"],
    material: "Diamond-plate steel",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "5-point system",
      insulation: "0.36 U-value",
      soundReduction: "29-34 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d15.jpg",
      "/assets/images/doors/d16.jpg",
      "/assets/images/doors/d17.jpg",
      "/assets/images/doors/d18.jpg",
      "/assets/images/doors/d19.jpg"
    ],
    features: [
      "Diamond plate pattern design",
      "Industrial aesthetic",
      "Modern styling",
      "Visual interest",
      "Premium appearance"
    ]
  },
  {
    id: 16,
    image: "/assets/images/doors/d16.jpg",
    name: "Granite Look Door",
    model: "DG-716",
    description: "Granite texture finish for sophisticated styling",
    price: "₹28,900",
    category: "Textured Doors",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2c3e3e", "#3d4d4d", "#4d5d5d"],
    material: "Granite-textured steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "6-point system",
      insulation: "0.37 U-value",
      soundReduction: "30-35 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d16.jpg",
      "/assets/images/doors/d17.jpg",
      "/assets/images/doors/d18.jpg",
      "/assets/images/doors/d19.jpg",
      "/assets/images/doors/d20.jpg"
    ],
    features: [
      "Granite texture appearance",
      "Sophisticated styling",
      "Non-slip surface",
      "Professional look",
      "Premium finish"
    ]
  },
  {
    id: 17,
    image: "/assets/images/doors/d17.jpg",
    name: "Linear Groove Door",
    model: "DG-717",
    description: "Contemporary linear groove pattern for modern spaces",
    price: "₹26,500",
    category: "Modern Doors",
    dimensions: ["850 x 2100 x 48 mm", "900 x 2200 x 50 mm", "1000 x 2400 x 55 mm"],
    sizes: ["830", "880", "980"],
    colors: ["#3d3d3d", "#2c2c2c", "#4a4a4a"],
    material: "Grooved-pattern steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.40 U-value",
      soundReduction: "29-33 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d17.jpg",
      "/assets/images/doors/d18.jpg",
      "/assets/images/doors/d19.jpg",
      "/assets/images/doors/d20.jpg",
      "/assets/images/doors/d21.jpg"
    ],
    features: [
      "Linear groove pattern",
      "Contemporary design",
      "Visual dimension",
      "Modern appeal",
      "Easy to maintain"
    ]
  },
  {
    id: 18,
    image: "/assets/images/doors/d18.jpg",
    name: "Shadowline Detail Door",
    model: "DG-718",
    description: "Subtle shadowline detail adds depth and elegance",
    price: "₹29,300",
    category: "Premium Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2b2b2b", "#3d3d3d", "#4a4a4a"],
    material: "Detailed-edge steel",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "6-point system",
      insulation: "0.38 U-value",
      soundReduction: "31-36 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d18.jpg",
      "/assets/images/doors/d19.jpg",
      "/assets/images/doors/d20.jpg",
      "/assets/images/doors/d21.jpg",
      "/assets/images/doors/d22.jpg"
    ],
    features: [
      "Shadowline edge detail",
      "Premium appearance",
      "Sophisticated design",
      "Visual depth",
      "Luxury styling"
    ]
  },
  {
    id: 19,
    image: "/assets/images/doors/d19.jpg",
    name: "Vertical Line Door",
    model: "DG-719",
    description: "Vertical line design for tall and elegant spaces",
    price: "₹25,800",
    category: "Decorative Doors",
    dimensions: ["900 x 2100 x 50 mm", "950 x 2150 x 52 mm", "1000 x 2200 x 55 mm"],
    sizes: ["880", "930", "980"],
    colors: ["#3d3d3d", "#4a4a4a", "#5a5a5a"],
    material: "Vertical-line steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.39 U-value",
      soundReduction: "28-32 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d19.jpg",
      "/assets/images/doors/d20.jpg",
      "/assets/images/doors/d21.jpg",
      "/assets/images/doors/d22.jpg",
      "/assets/images/doors/d23.jpg"
    ],
    features: [
      "Vertical line pattern",
      "Elongated design appearance",
      "Contemporary styling",
      "Mark-resistant",
      "Professional finish"
    ]
  },
  {
    id: 20,
    image: "/assets/images/doors/d20.jpg",
    name: "Stainless Steel Premium",
    model: "DG-720",
    description: "High-end stainless steel door for luxury spaces",
    price: "₹42,800",
    category: "Luxury Doors",
    dimensions: ["900 x 2100 x 60 mm", "1000 x 2200 x 65 mm", "1100 x 2400 x 70 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#c0c0c0", "#b8b8b8"],
    material: "Grade 304 stainless steel",
    specifications: {
      frameThickness: "2.0-2.3 mm",
      lockPoints: "7-point system",
      insulation: "0.28 U-value",
      soundReduction: "35-40 dB",
      fireRating: "90 min optional"
    },
    gallery: [
      "/assets/images/doors/d20.jpg",
      "/assets/images/doors/d21.jpg",
      "/assets/images/doors/d22.jpg",
      "/assets/images/doors/d23.jpg",
      "/assets/images/doors/d24.jpg"
    ],
    features: [
      "Premium stainless steel",
      "Corrosion-resistant",
      "Mirror finish available",
      "Luxury appearance",
      "Lifetime durability"
    ]
  },
  {
    id: 21,
    image: "/assets/images/doors/d21.jpg",
    name: "Fire-Rated Security",
    model: "DG-721",
    description: "Fire-resistant door with enhanced security features",
    price: "₹45,500",
    category: "Safety Doors",
    dimensions: ["900 x 2100 x 70 mm", "1000 x 2200 x 75 mm", "1100 x 2400 x 80 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2b2b2b", "#4a4a4a"],
    material: "Fire-rated steel",
    specifications: {
      frameThickness: "2.5-2.8 mm",
      lockPoints: "9-point system",
      insulation: "0.20 U-value",
      soundReduction: "42-48 dB",
      fireRating: "120 min certified"
    },
    gallery: [
      "/assets/images/doors/d21.jpg",
      "/assets/images/doors/d22.jpg",
      "/assets/images/doors/d23.jpg",
      "/assets/images/doors/d24.jpg",
      "/assets/images/doors/d25.jpg"
    ],
    features: [
      "Fire-resistant rating",
      "120-minute fire protection",
      "Smoke seal gaskets",
      "Emergency push-bar option",
      "Certified safety"
    ]
  },
  {
    id: 22,
    image: "/assets/images/doors/d22.jpg",
    name: "Soundproof Studio Door",
    model: "DG-722",
    description: "Advanced sound dampening technology for studios and offices",
    price: "₹39,800",
    category: "Acoustic Doors",
    dimensions: ["900 x 2100 x 60 mm", "1000 x 2200 x 65 mm", "1100 x 2400 x 70 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2c2c2c", "#3d3d3d", "#4a4a4a"],
    material: "Sound-dampening composite steel",
    specifications: {
      frameThickness: "2.2-2.5 mm",
      lockPoints: "8-point system",
      insulation: "0.25 U-value",
      soundReduction: "48-52 dB",
      fireRating: "90 min optional"
    },
    gallery: [
      "/assets/images/doors/d22.jpg",
      "/assets/images/doors/d23.jpg",
      "/assets/images/doors/d24.jpg",
      "/assets/images/doors/d25.jpg",
      "/assets/images/doors/d26.jpg"
    ],
    features: [
      "Advanced sound insulation",
      "50dB noise reduction",
      "Acoustic sealing gaskets",
      "Professional studio grade",
      "Perfect for offices"
    ]
  },
  {
    id: 23,
    image: "/assets/images/doors/d23.jpg",
    name: "Eco-Green Door",
    model: "DG-723",
    description: "Environmentally friendly sustainable door option",
    price: "₹24,500",
    category: "Eco Doors",
    dimensions: ["850 x 2100 x 50 mm", "900 x 2200 x 55 mm", "1000 x 2400 x 60 mm"],
    sizes: ["830", "880", "980"],
    colors: ["#3d4d3d", "#4d5d4d", "#5d6d5d"],
    material: "Recycled steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.41 U-value",
      soundReduction: "27-31 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d23.jpg",
      "/assets/images/doors/d24.jpg",
      "/assets/images/doors/d25.jpg",
      "/assets/images/doors/d26.jpg",
      "/assets/images/doors/d27.jpg"
    ],
    features: [
      "100% recyclable material",
      "Eco-friendly production",
      "Sustainable choice",
      "Lower carbon footprint",
      "Green certification"
    ]
  },
  {
    id: 24,
    image: "/assets/images/doors/d24.jpg",
    name: "Rose Gold Elegant",
    model: "DG-724",
    description: "Trendy rose gold finish for modern luxury homes",
    price: "₹30,500",
    category: "Colored Premium",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#c08070", "#d8956d", "#b8795d"],
    material: "Rose-gold coated steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "6-point system",
      insulation: "0.37 U-value",
      soundReduction: "30-35 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d24.jpg",
      "/assets/images/doors/d25.jpg",
      "/assets/images/doors/d26.jpg",
      "/assets/images/doors/d27.jpg",
      "/assets/images/doors/d28.jpg"
    ],
    features: [
      "Trendy rose gold finish",
      "Premium color coating",
      "Modern aesthetic",
      "Fashion-forward design",
      "Luxury appeal"
    ]
  },
  {
    id: 25,
    image: "/assets/images/doors/d25.jpg",
    name: "Copper Metallic Door",
    model: "DG-725",
    description: "Distinctive copper metallic finish for unique spaces",
    price: "₹31,200",
    category: "Metallic Doors",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#b87333", "#a0622d", "#8a5a2a"],
    material: "Copper-coated steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "6-point system",
      insulation: "0.37 U-value",
      soundReduction: "30-35 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d25.jpg",
      "/assets/images/doors/d26.jpg",
      "/assets/images/doors/d27.jpg",
      "/assets/images/doors/d28.jpg",
      "/assets/images/doors/d29.jpg"
    ],
    features: [
      "Distinctive copper metallic",
      "Anti-patina coating",
      "Unique appearance",
      "Premium styling",
      "Architectural design"
    ]
  },
  {
    id: 26,
    image: "/assets/images/doors/d26.jpg",
    name: "Contemporary Matte Grey",
    model: "DG-726",
    description: "Sophisticated matte grey for contemporary homes",
    price: "₹26,200",
    category: "Contemporary",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#666666", "#767676", "#777777"],
    material: "Matte-finish steel",
    specifications: {
      frameThickness: "1.6-1.8 mm",
      lockPoints: "5-point system",
      insulation: "0.39 U-value",
      soundReduction: "28-32 dB",
      fireRating: "30 min optional"
    },
    gallery: [
      "/assets/images/doors/d26.jpg",
      "/assets/images/doors/d27.jpg",
      "/assets/images/doors/d28.jpg",
      "/assets/images/doors/d29.jpg",
      "/assets/images/doors/d30.jpg"
    ],
    features: [
      "Matte grey finish",
      "Contemporary styling",
      "Fingerprint-resistant",
      "Modern appearance",
      "Versatile color"
    ]
  },
  {
    id: 27,
    image: "/assets/images/doors/d27.jpg",
    name: "Geometric Pattern Door",
    model: "DG-727",
    description: "Modern geometric pattern design for artistic spaces",
    price: "₹28,600",
    category: "Designer Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#3d3d3d", "#4a4a4a", "#5a5a5a"],
    material: "Patterned-surface steel",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "5-point system",
      insulation: "0.36 U-value",
      soundReduction: "29-34 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d27.jpg",
      "/assets/images/doors/d28.jpg",
      "/assets/images/doors/d29.jpg",
      "/assets/images/doors/d30.jpg",
      "/assets/images/doors/d31.jpg"
    ],
    features: [
      "Modern geometric pattern",
      "Artistic design",
      "Visual interest",
      "Contemporary appeal",
      "Unique styling"
    ]
  },
  {
    id: 28,
    image: "/assets/images/doors/d28.jpg",
    name: "Wave Texture Door",
    model: "DG-728",
    description: "Flowing wave texture pattern for modern elegance",
    price: "₹27,900",
    category: "Textured Premium",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#4a4a4a", "#3d3d3d", "#5a5a5a"],
    material: "Wave-textured steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "6-point system",
      insulation: "0.38 U-value",
      soundReduction: "31-36 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d28.jpg",
      "/assets/images/doors/d29.jpg",
      "/assets/images/doors/d30.jpg",
      "/assets/images/doors/d31.jpg",
      "/assets/images/doors/d32.jpg"
    ],
    features: [
      "Flowing wave texture",
      "Premium appearance",
      "Elegant pattern",
      "Hides marks well",
      "Professional finish"
    ]
  },
  {
    id: 29,
    image: "/assets/images/doors/d29.jpg",
    name: "Hexagon Pattern Door",
    model: "DG-729",
    description: "Modern hexagon pattern for contemporary design lovers",
    price: "₹29,100",
    category: "Geometric Doors",
    dimensions: ["900 x 2100 x 50 mm", "1000 x 2200 x 55 mm", "1100 x 2400 x 60 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#2c2c2c", "#3d3d3d", "#4a4a4a"],
    material: "Hexagon-patterned steel",
    specifications: {
      frameThickness: "1.7-1.9 mm",
      lockPoints: "5-point system",
      insulation: "0.36 U-value",
      soundReduction: "29-34 dB",
      fireRating: "45 min optional"
    },
    gallery: [
      "/assets/images/doors/d29.jpg",
      "/assets/images/doors/d30.jpg",
      "/assets/images/doors/d31.jpg",
      "/assets/images/doors/d32.jpg",
      "/assets/images/doors/d33.jpg"
    ],
    features: [
      "Hexagon geometric pattern",
      "Modern design",
      "Architectural interest",
      "Contemporary styling",
      "Visual appeal"
    ]
  },
  {
    id: 30,
    image: "/assets/images/doors/d30.jpg",
    name: "Embossed Detail Door",
    model: "DG-730",
    description: "Subtle embossed details for sophisticated interiors",
    price: "₹28,300",
    category: "Premium Detailed",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#3d3d3d", "#4a4a4a", "#5a5a5a"],
    material: "Embossed-surface steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "6-point system",
      insulation: "0.37 U-value",
      soundReduction: "30-35 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d30.jpg",
      "/assets/images/doors/d31.jpg",
      "/assets/images/doors/d32.jpg",
      "/assets/images/doors/d33.jpg",
      "/assets/images/doors/d1.jpg"
    ],
    features: [
      "Subtle embossed details",
      "Sophisticated appearance",
      "Premium styling",
      "Tactile surface",
      "Professional design"
    ]
  },
  {
    id: 31,
    image: "/assets/images/doors/d31.jpg",
    name: "Ultra-Modern Black",
    model: "DG-731",
    description: "Sleek ultra-modern black design for contemporary spaces",
    price: "₹33,200",
    category: "Ultra-Modern",
    dimensions: ["900 x 2100 x 55 mm", "1000 x 2200 x 60 mm", "1100 x 2400 x 65 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#0a0a0a", "#1a1a1a", "#2b2b2b"],
    material: "Ultra-gloss black steel",
    specifications: {
      frameThickness: "1.8-2.0 mm",
      lockPoints: "7-point system",
      insulation: "0.38 U-value",
      soundReduction: "32-37 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d31.jpg",
      "/assets/images/doors/d32.jpg",
      "/assets/images/doors/d33.jpg",
      "/assets/images/doors/d1.jpg",
      "/assets/images/doors/d2.jpg"
    ],
    features: [
      "Ultra-modern design",
      "Glossy black finish",
      "Contemporary styling",
      "Sleek appearance",
      "Premium black option"
    ]
  },
  {
    id: 32,
    image: "/assets/images/doors/d32.jpg",
    name: "Champagne Gloss Door",
    model: "DG-732",
    description: "Luxurious champagne gloss finish for elegant spaces",
    price: "₹34,500",
    category: "Luxury Finishes",
    dimensions: ["900 x 2100 x 60 mm", "1000 x 2200 x 65 mm", "1100 x 2400 x 70 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#e6d5c3", "#f0e0d0"],
    material: "Champagne-gloss steel",
    specifications: {
      frameThickness: "1.9-2.1 mm",
      lockPoints: "7-point system",
      insulation: "0.36 U-value",
      soundReduction: "31-36 dB",
      fireRating: "60 min optional"
    },
    gallery: [
      "/assets/images/doors/d32.jpg",
      "/assets/images/doors/d33.jpg",
      "/assets/images/doors/d1.jpg",
      "/assets/images/doors/d2.jpg",
      "/assets/images/doors/d3.jpg"
    ],
    features: [
      "Luxurious champagne gloss",
      "Premium finish",
      "Elegant appearance",
      "High-shine surface",
      "Sophisticated luxury"
    ]
  },
  {
    id: 33,
    image: "/assets/images/doors/d33.jpg",
    name: "Ultimate Security Pro",
    model: "DG-733",
    description: "Ultimate security door with advanced locking and reinforced steel",
    price: "₹46,800",
    category: "Maximum Security",
    dimensions: ["900 x 2100 x 80 mm", "1000 x 2200 x 85 mm", "1100 x 2400 x 90 mm"],
    sizes: ["880", "980", "1080"],
    colors: ["#1a1a1a", "#2b2b2b", "#3d3d3d"],
    material: "Military-grade steel",
    specifications: {
      frameThickness: "2.4-2.8 mm",
      lockPoints: "12-point system",
      insulation: "0.22 U-value",
      soundReduction: "45-50 dB",
      fireRating: "120 min certified"
    },
    gallery: [
      "/assets/images/doors/d33.jpg",
      "/assets/images/doors/d1.jpg",
      "/assets/images/doors/d2.jpg",
      "/assets/images/doors/d3.jpg",
      "/assets/images/doors/d4.jpg"
    ],
    features: [
      "Military-grade steel construction",
      "Advanced biometric lock ready",
      "Triple-point locking system",
      "Reinforced steel core",
      "Maximum security certification"
    ]
  }
];

